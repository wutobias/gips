...
class mode6(MC_fitter):
    ...
    def __init__(self, gdatarec_dict,
        ...
        super(mode6, self).__init__(gdatarec_dict=gdatarec_dict,
    ...
    def _f_process(self, x):
        ...
        if self.anal_grad:
            ...
            if self.pairs:
                ...
                _g  = merge_casedata_ext(self._gradients_cplx[:,-3,0], self.ind_case_cplx, self.w_cplx, self.ind_case_cplx)
                self._g[:,-4,0] += pair_difference_ext(_g, self.pairidx)
                _g  = merge_casedata_ext(self._gradients_lig[:, -3,0], self.ind_case_lig,  self.w_lig,  self.ind_case_lig)
                self._g[:,-3,0] -= pair_difference_ext(_g, self.pairidx)
                _g  = merge_casedata_ext(self._gradients_cplx[:,-3,1], self.ind_case_cplx, self.w_cplx, self.ind_case_cplx)
                self._g[:,-4,1] += pair_difference_ext(_g, self.pairidx)
                _g  = merge_casedata_ext(self._gradients_lig[:, -3,1], self.ind_case_lig,  self.w_lig,  self.ind_case_lig)
                ...
                self._g[:,-4,0] += merge_casedata_ext(self._gradients_cplx[:,-3,0], self.ind_case_cplx, self.w_cplx, self.ind_case_cplx)
                self._g[:,-5,0] -= merge_casedata_ext(self._gradients[:,-3,0],      self.ind_case, self.w, self.ind_rec)
                self._g[:,-3,0] -= merge_casedata_ext(self._gradients_lig[:,-3,0], self.ind_case_lig,  self.w_lig,  self.ind_case_lig)
                self._g[:,-4,1] += merge_casedata_ext(self._gradients_cplx[:,-3,1], self.ind_case_cplx, self.w_cplx, self.ind_case_cplx)
                self._g[:,-5,1] -= merge_casedata_ext(self._gradients[:,-3,1],      self.ind_case, self.w, self.ind_rec)
                self._g[:,-3,1] -= merge_casedata_ext(self._gradients_lig[:,-3,1], self.ind_case_lig,  self.w_lig,  self.ind_case_lig)
    ...
    def set_bounds(self):
        ...
        if isinstance(self.boundsdict, dict):
            if not self.pairs:
                self.xmin[-5], self.xmax[-5] = self.boundsdict['g_co'][0], self.boundsdict['g_co'][1] ### g_co (Rec)
                self.kforce_f[-5] = 10.
            self.xmin[-4], self.xmax[-4] = self.boundsdict['g_co'][0], self.boundsdict['g_co'][1] ### g_co (Cplx)
            self.xmin[-3], self.xmax[-3] = self.boundsdict['g_co'][0], self.boundsdict['g_co'][1] ### g_co (Lig)
            ...
            if not self.pairs:
                self.xmin[-5], self.xmax[-5] = np.min(_g_r), np.max(_g_r) ### g_co (Rec)
                self.kforce_f[-5] = 10.
            self.xmin[-4], self.xmax[-4] = np.min(_g_c), np.max(_g_c) ### g_co (Cplx)
            self.xmin[-3], self.xmax[-3] = np.min(_g_l), np.max(_g_l) ### g_co (Lig)
