...
class mode0(MC_fitter):
    ...
    def __init__(self, gdatarec_dict, 
        ...
        super(mode0, self).__init__(gdatarec_dict=gdatarec_dict,
    ...
    def _f_process(self, x):
        ...
        if self.anal_grad:
            ...
            if self.pairs:
                _f          = merge_casedata_ext(self._calc_data, self.ind_case, self.w, self.ind_rec)
                self._f[:] += pair_difference_ext(_f, self.pairidx)+x[-1]

                for i in range(self.parms-1):
                    _g            = merge_casedata_ext(self._gradients[:,i], self.ind_case, self.w, self.ind_rec)
